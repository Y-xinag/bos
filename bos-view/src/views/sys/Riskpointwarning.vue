<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.search" placeholder="请输入标题名" style="width: 200px;" class="filter-item" />
      <!--      @click="getList"-->
    ...
    >
      ...
      <el-table-column label="标题" min-width="200px">
        <template slot-scope="{row}">
          <span class="link-type">{{ row.wtitle }}</span>
        </template>
      </el-table-column>

      <el-table-column label="创建人" min-width="200px">
        <template slot-scope="{row}">
          <span class="link-type">{{ row.wcreatename }}</span>
        </template>
      </el-table-column>

      <el-table-column label="创建时间" width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.wcreatetime | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
